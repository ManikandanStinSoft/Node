...
const createHomeLoan = async (req, res) => {
  ...
  try {
    ...
    const loanAmountRequested = parseFloat(req.body.loan_amount_requested);
    const interestAmount = parseFloat(req.body.interest_amount);
    const noEmi = parseInt(req.body.no_emi, 10);
    ...
    const checkQuery = `SELECT COUNT(*) AS count FROM VehicleLoan WHERE email = ? OR mobile = ?`;
    ...
    const insertQuery = `
      INSERT INTO VehicleLoan (
        is_verified, created_employee_name,created_employee_id, firstname, lastname, gender, date_of_birth, age, mobile,
        alternate_mobile, email, address_1, address_2, city, state, pincode, aadhar_number, pan_number,
        ...
        guarantor_gender, guarantor_date_of_birth,guarantor_age, guarantor_mobile, guarantor_alternate_mobile, 
        guarantor_email, guarantor_address1, guarantor_address2, guarantor_state, guarantor_city, guarantor_pincode,
        guarantor_aadhar_number, guarantor_pan_number, apply_for_loan_user, mark_as_read_by_user, send_to_employee,
        send_to_employee_date, send_approval, send_approval_date, mark_as_read_employee, remarks, is_rejected,
        approval_date, rejected_date, mark_as_read_admin, rejected_reason_admin, approval_date_admin, rejected_date_admin, 
        assigned_to, image, aadhar_image, pan_image, signature,guarantor_image, guarantor_aadhar_image, 
        guarantor_pan_image, guarantor_signature
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
    ...
    const formId = `VL${day}${month}${year}${loanId}`;

    const updateQuery = `UPDATE VehicleLoan SET form_id = ? WHERE id = ?`;
    await connection.query(updateQuery, [formId, loanId]);

    const folderPath = `Finance/VehicleLoan/${formId}`;
    if (files.image) {
    ...
    await connection.query(updateFilesQuery, updateFilesValues);

    await connection.commit();

    res.status(201).json({
      message: "Vehicle Loan created successfully",
      loanId: loanId,
    });
  } catch (error) {
    ...
    res
      .status(500)
      .json({ message: "Error creating Vehicle Loan", error: error.message });
  } finally {
    connection.release();
  }
};
...
const updateHomeLoan = async (req, res) => {
  try {
    ...
    const [existingData] = await pool.query(
      `SELECT * FROM VehicleLoan WHERE form_id = ?`,
      [loanId]
    );
    ...
    const folderPath = `Finance/VehicleLoan/${loanId}`;
    try {
      if (files.image) {
        loanData.image = await uploadFileToSpaces(
          files.image[0],
    ...
    const checkQuery = `SELECT COUNT(*) AS count FROM VehicleLoan WHERE (email = ? OR mobile = ?) AND form_id != ?`;
    ...
    res.status(200).json({ message: "Home loan updated successfully" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "Error updating home loan", error });
  }
};
...
const deleteHomeLoan = async (req, res) => {
  try {
    ...
    const query = "DELETE FROM VehicleLoan WHERE form_id = ?";
    const [result] = await pool.query(query, [loanId]);

    if (result.affectedRows === 0) {
      return res.status(404).json({ message: "Vehicle Loan record not found" });
    }

    res
      .status(200)
      .json({ message: "Vehicle Loan record deleted successfully" });
  } catch (error) {
    res.status(500).json({
      message: "Error deleting Vehicle Loan record",
      error: error.message,
    });
  }
};
...
const getAllHomeLoans = async (req, res) => {
  try {
    const query = "SELECT * FROM VehicleLoan";
    ...
    res.status(500).json({
      message: "Error fetching Vehicle Loan records",
      error: error.message,
    });
  }
};

const getHomeLoanById = async (req, res) => {
  const { form_id } = req.params;

  try {
    const query = "SELECT * FROM VehicleLoan WHERE form_id = ?";
    ...
    res.status(500).json({
      message: "Error fetching Vehicle Loan by ID",
      error: error.message,
    });
  }
};

const getHomeLoanByEmail = async (req, res) => {
  try {
    const email = req.params.email;
    const query = "SELECT * FROM VehicleLoan WHERE email = ?";
    const [results] = await pool.query(query, [email]);

    if (results.length === 0) {
      return res
        .status(404)
        .json({ message: "No Vehicle Loan found for this email" });
    }

    res.status(200).json(results);
  } catch (error) {
    res.status(500).json({
      message: "Error fetching Vehicle Loan by email",
      error: error.message,
    });
  }
};

const getHomeLoanByMobile = async (req, res) => {
  try {
    const mobile = req.params.mobile;
    const query = "SELECT * FROM VehicleLoan WHERE mobile = ?";
    const [results] = await pool.query(query, [mobile]);

    if (results.length === 0) {
      return res
        .status(404)
        .json({ message: "No Vehicle Loan found for this mobile number" });
    }

    res.status(200).json(results);
  } catch (error) {
    res.status(500).json({
      message: "Error fetching Vehicle Loan by mobile",
      error: error.message,
    });
  }
};

const getHomeLoanByAdminId = async (req, res) => {
  const { adminId, adminName } = req.params;
  try {
    const loans = await HomeLoanModel.getHomeLoanByAdminId(adminId, adminName);
    ...
    res.status(500).json({
      message: "Error fetching Vehicle Loan by admin ID",
      error: err.message,
    });
  }
};

const getApprovedVehicleLoans = async (req, res) => {
  try {
    const sendApprovalStatus = "No";
    const query = "SELECT * FROM VehicleLoan WHERE send_approval = ?";

    const [results] = await pool.query(query, [sendApprovalStatus]);

    if (Array.isArray(results) && results.length === 0) {
      return res
        .status(404)
        .json({ message: "No approved Vehicle Loans found" });
    }

    res.status(200).json(results);
  } catch (error) {
    res.status(500).json({
      message: "Error fetching approved Vehicle Loans",
      error: error.message,
    });
  }
};
...
const getLoanStatusApprovedCount = async (req, res) => {
  ...
  try {
    const approvedLoans = await HomeLoanModel.getLoanStatusApprovedCount(
      adminId.trim(),
      adminName.trim(),
      adminRole.trim()
    );
...
const getLoanStatusPendingCount = async (req, res) => {
  const { adminId, adminName, adminRole } = req.params;
  try {
    const pendingLoans = await HomeLoanModel.getLoanStatusPendingCount(
      adminId.trim(),
      adminName.trim(),
      adminRole.trim()
    );
...
const getLoanStatusRejectedCount = async (req, res) => {
  const { adminId, adminName, adminRole } = req.params;
  try {
    const rejectedLoans = await HomeLoanModel.getLoanStatusRejectedCount(
      adminId.trim(),
      adminName.trim(),
      adminRole.trim()
    );
...
const getLoanStatusInProgressCount = async (req, res) => {
  const { adminId, adminName, adminRole } = req.params;
  try {
    const inProgressLoans = await HomeLoanModel.getLoanStatusInProgressCount(
      adminId.trim(),
      adminName.trim(),
      adminRole.trim()
    );
...
const getLoanStatusClosedCount = async (req, res) => {
  const { adminId, adminName, adminRole } = req.params;
  try {
    const closedLoans = await HomeLoanModel.getLoanStatusClosedCount(
      adminId.trim(),
      adminName.trim(),
      adminRole.trim()
    );
...
const getApprovedHomeLoansYes = async (req, res) => {
  try {
    const query = "SELECT * FROM VehicleLoan WHERE send_approval = 'Yes'";
    const [results] = await pool.query(query);

    if (results.length === 0) {
      return res.status(404).json({
        message: "No home loans found with approval status 'Yes'.",
      });
    }

    res.status(200).json(results);
  } catch (error) {
    console.error("Error fetching home loans with 'Yes' approval:", error);
    res.status(500).json({
      message: "Error fetching home loans with 'Yes' approval status.",
      error: error.message,
    });
  }
};

const getApprovedHomeLoansNo = async (req, res) => {
  try {
    const query = "SELECT * FROM VehicleLoan WHERE send_approval = 'No'";
    const [results] = await pool.query(query);

    if (results.length === 0) {
      return res.status(404).json({
        message: "No home loans found with approval status 'No'.",
      });
    }

    res.status(200).json(results);
  } catch (error) {
    console.error("Error fetching home loans with 'No' approval:", error);
    res.status(500).json({
      message: "Error fetching home loans with 'No' approval status.",
      error: error.message,
    });
  }
};

const getHomeLoanApprovedLoans = async (req, res) => {
  const { adminId, adminName, adminRole } = req.params;

  // console.log("Received Admin Details:");
  // console.log("Admin ID:", adminId);
  // console.log("Admin Name:", adminName);
  // console.log("Admin Role:", adminRole);

  try {
    const loans = await HomeLoanModel.getHomeLoanByApprovedLoanStatus(
      adminId.trim(),
      adminName.trim(),
      adminRole.trim()
    );
    ...
    res.status(500).json({
      message: "Error fetching Vehicle Loan by loan status",
      error: err.message,
    });
  }
};

const getHomeLoanPendingLoans = async (req, res) => {
  const { adminId, adminName, adminRole } = req.params;
  ...
  try {
    const loans = await HomeLoanModel.getHomeLoanByPendingLoanStatus(
      adminId.trim(),
      adminName.trim(),
      adminRole.trim()
    );
...
const getHomeLoanRejectLoans = async (req, res) => {
  const { adminId, adminName, adminRole } = req.params;
  ...
  try {
    const loans = await HomeLoanModel.getHomeLoanByRejectLoanStatus(
      adminId.trim(),
      adminName.trim(),
      adminRole.trim()
    );
...
const getHomeLoanInProgressLoans = async (req, res) => {
  const { adminId, adminName, adminRole } = req.params;
  ...
  try {
    const loans = await HomeLoanModel.getHomeLoanByInProgressLoanStatus(
      adminId.trim(),
      adminName.trim(),
      adminRole.trim()
    );
...
const getHomeLoanOpenLoans = async (req, res) => {
  const { adminId, adminName, adminRole } = req.params;
  ...
  try {
    const loans = await HomeLoanModel.getHomeLoanByOpenLoanStatus(
      adminId.trim(),
      adminName.trim(),
      adminRole.trim()
    );
...
const updateApprovedVehicleLoanStatus = async (req, res) => {
  ...
  try {
    ...
    const result = await HomeLoanModel.updateApprovedVehicleLoanStatus(
      loanId,
      loan_status
    );
...
const updateRejectedVehicleLoanStatus = async (req, res) => {
  console.log("Request received for form ID:", req.params.form_id);
  console.log("Request body:", req.body);
  ...
  try {
    ...
    const result = await HomeLoanModel.updateRejectedVehicleLoanStatus(
      loanId,
      loan_status
    );
...
const updateClosedVehicleLoanStatus = async (req, res) => {
  console.log("Request received for form ID:", req.params.form_id);
  console.log("Request body:", req.body);
  ...
  try {
    ...
    const result = await HomeLoanModel.updateClosedVehicleLoanStatus(
      loanId,
      loan_status
    );
...
const updateHomeLoanVerificationStatus = async (req, res) => {
  try {
    const loanId = req.params.form_id;
    const { 
      is_verified, 
    ...
    const result = await HomeLoanModel.updateLoanVerificationStatus(loanId, updateData);
...
const updateHomeLoanSendApprovalStatus = async (req, res) => {
  try {
    const loanId = req.params.form_id;
...
const getUnclosedHomeLoan = async (req, res) => {
  const { adminId, adminName, adminRole } = req.params;
  try {
    ...
    const vehicleLoanQuery = `
      SELECT * FROM VehicleLoan
      WHERE form_id IN (?)
    `;
    const [vehicleLoanResults] = await pool.query(vehicleLoanQuery, [formIds]);

    console.log("Query Result for Vehicle Loans:");
    console.log(vehicleLoanResults);

    res.status(200).json({
      message: "success",
      data: vehicleLoanResults,
    });
  } catch (err) {
    console.error("Error fetching closed vehicle loan records:", err.message);
    ...
    res.status(500).json({
      message: "Error fetching closed vehicle loan records",
      error: err.message,
    });
  }
};

const saveRemarksForVehicleLoanController = async (req, res) => {
  ...
  try {
    ...
    const result = await HomeLoanModel.saveRemarksForVehicleLoan(loanId, remarks);
    return res.status(200).json({
...
module.exports = {
  ...
  getApprovedHomeLoansYes,
  getApprovedHomeLoansNo,
  getHomeLoanApprovedLoans,
  getHomeLoanPendingLoans,
  getHomeLoanRejectLoans,
  getHomeLoanInProgressLoans,
  getHomeLoanOpenLoans,
  updateApprovedVehicleLoanStatus,
  updateRejectedVehicleLoanStatus,
  updateClosedVehicleLoanStatus,
  updateHomeLoanVerificationStatus,
  updateHomeLoanSendApprovalStatus,
  getUnclosedHomeLoan,
  saveRemarksForVehicleLoanController
};
